---
name: mcs-research
description: "Use this skill to research and design an agent before building it. Reads project documents, identifies agents, researches MCS components, scores architecture, writes instructions, and generates eval sets. Use after /mcs-init, when documents change, when the user wants to analyze requirements, or when agentspec.json needs enrichment."
---
...
- Fast-A1: Extract per-agent data -> write to agentspec.json
- Fast-A2: Generate open questions
- Fast-A3: Generate eval stubs

**Writes to agentspec.json:**
...
- First run: reads all docs, identifies agents, deep research, creates agentspec.json with evalSets + evals.csv
...
- Spec edited (open questions answered): re-enriches with new context even without new docs

## Output Files (per agent)

- `Build-Guides/{projectId}/agents/{agentId}/agentspec.json` -- Single source of truth (all fields populated including instructions + evalSets)
...
## Wizard-Created Specs (Enrichment-Aware)

When an agent spec was created via the Agent Wizard (`/create` route), it may already have:
...
**If enrichment has already run** (check `agentspec.json._enrichment`):
- Skip Phase A extraction (spec already populated from wizard conversation)
...
**If enrichment has not run** (no `_enrichment` field, or wizard spec without enrichment):
...
**Goal:** Determine the optimal processing path for any invocation -- project or agent level. Detects new/changed docs, spec edits, and manually created agents. This phase runs for all invocations because accurate routing prevents wasted work and missed updates.

> Full routing protocol with all 7 substeps: see `reference/smart-routing.md`

**Key steps:** Determine scope -> check preconditions (manifest + spec) -> diff docs against manifest -> map docs to agents -> check spec modifications -> detect drastic changes -> route and report.
...
**Goal:** Read all project documents, build a unified understanding, identify every agent to build, and create agentspec.json stubs with informed open questions. This is deep comprehension, not surface extraction.
...
9. Create agentspec.json stubs, confirm with user, write document manifest
10. **Speculative Research Analyst dispatch** (background): If any extracted integration is unfamiliar (not in `knowledge/cache/connectors.md`, `mcp-servers.md`, or `first-party-agents.md`), spawn `research-analyst` with `run_in_background: true` to scan for Priority 5-6 systems. RA writes findings to `Build-Guides/<project>/agents/<name>/research-analyst.findings.json`. Phase B Step 4 reads this file before deciding whether to do its own RA spawn.
...
3. **Stable categories from cache (Step 1):** Channels, triggers, knowledge sources -- read cache directly, write to spec.
...
| Teammate | Input | Output | Skip When | Isolation |
|----------|-------|--------|-----------|-----------|
| **Prompt Engineer** | agentspec.json, instructions-authoring cache, model selection | Instructions (up to 8K chars, self-verified) | Incremental + instructions exist | `worktree` |
| **QA Challenger** | agentspec.json, eval-guide plan + generated tests (lead-invoked), learnings | 3 eval sets (boundaries/quality/edge-cases), 40-55 tests, coverage report | -- | `worktree` |
| **Topic Engineer** | agentspec.json topics, capabilities, integrations, adaptive-cards + conversation-design cache | Per-topic feasibility (OK / SPLIT / caveats) | No custom topics | `worktree` |
| **Flow Designer** | agentspec.json (flow capabilities), integrations, architecture | flow-spec.md | solutionType ∉ {flow, hybrid, automation} | `worktree` |

All four dispatch in **a single message with multiple Agent tool calls** so they run simultaneously. Each gets `isolation: "worktree"` so concurrent writes to `agentspec.json` and sidecar files don't collide. Teammates that make no changes have their worktree auto-cleaned; teammates that wrote files leave the worktree behind for the lead to read and merge.

Every teammate fires GPT-5.5 internally for co-gen and review (PE: `generate-instructions`, QA: `generate-evals` for co-gen merge, TE: `generate-topics` for 3+ node topics, FD: `generate-flow` + `review-flow`). The **lead** invokes Skill-tool plugins (teammates have no Skill access) — for Phase C eval generation this means the lead runs eval-guide `/eval-suite-planner` + `/eval-generator` and hands the plan + tests to QA for bucket mapping and normalization. Teammates handle GPT merging within their worktree; the lead reads the merged output and applies it to the main checkout.

### Lead Reconciliation (Step 3)

After teammates return: apply PE instructions with inline review (9-point checklist), apply QA eval sets + coverage review, apply TE split/caveat recommendations, generate per-set CSVs (evals-boundaries.csv, evals-quality.csv, evals-edge-cases.csv), write all build-ready data to agentspec.json.

**EvalTest field names are strict** — use `question` (NOT `input`), `expected` (NOT `expectedOutput`), `methods` (array, NOT `method` object), `scenarioId` (NOT `scenario`), `lastResult: null` (REQUIRED). See `templates/agentspec.json` lines 239-253. Wrong names cause the dashboard to show empty strings.

### GPT Parallel Review (Step 3.5)

After reconciliation, fire GPT-5.5 in parallel on all research outputs:
...
1. **agentspec.json** -- All fields populated (business, agent, capabilities, integrations, knowledge, conversations, boundaries, architecture, evalSets, evalConfig, mvpSummary, openQuestions, instructions, workflow)
...
{If new decisions: "New decisions added -- review in spec before building."}
...
Recommended defaults pre-applied to spec. Review and confirm before building.

Files: agentspec.json + evals.csv per agent

**Next:** Review spec in the dashboard. Resolve open questions and decisions. Then /mcs-build.
```

No report file is generated. The dashboard renders agentspec.json directly.
...
- **agentspec.json is the source of truth** -- the dashboard reads it, the build skill reads it, reports are generated from it
- **No separate agent-spec.md** -- everything lives in agentspec.json including instructions and MVP scope
- **evals.csv is for MCS native eval compatibility** -- flat export from agentspec.json evalSets. The Eval skill reads evalSets directly.
- **Only 2 permanent output files per agent**: `agentspec.json` and `evals.csv`. No working-paper files -- consolidate teammate outputs into agentspec.json fields and delete working files before completing.
...
- **Spec edits trigger re-enrichment** because answered questions provide new context even without new docs.
- **`full-agent` for manually created agents** -- empty spec + agent scope = full research scoped to that agent.
...
- **agentspec.json is the context** -- during incremental processing, read the spec for context instead of re-reading unchanged docs.
...
- **Pre-apply the recommended option** -- the spec should be buildable even if the user never reviews decisions, because the recommended option's `briefPatch` is written to spec fields as the default.
