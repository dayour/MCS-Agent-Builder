...
- **Issue**: Named `mcs-agent-builder` same as root. No conflict today (`private: true`) but confusing.
- **Fix**: Rename to `@mcs-agent-builder/frontend` or `mcs-agent-builder-dashboard`.
...
> Run `mcs-agent-builder health` to verify the dashboard is working after applying fixes.
