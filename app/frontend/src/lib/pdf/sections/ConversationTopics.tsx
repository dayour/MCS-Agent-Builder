...
const ConversationTopics = ({ data, sectionNumber }: Props) => {
  const items = data?.items;
  if (!items?.length) return null;

  return (
    <View>
      <SectionHeading number={sectionNumber} title="Conversation Topics" />

      <View style={s.table}>
        <View style={s.header} wrap={false}>
          <View style={{ flex: 2 }}><Text style={s.headerCell}>Topic</Text></View>
          <View style={{ flex: 3 }}><Text style={s.headerCell}>Description</Text></View>
          <View style={{ flex: 1 }}><Text style={s.headerCell}>Type</Text></View>
        ...
        {items.map((t: any, i: number) => (
          <View key={i} style={[s.row, i % 2 === 1 && s.rowAlt]} wrap={false}>
            ...
            <View style={{ flex: 3 }}>
              <Text style={s.cell}>{safe(t.description)}</Text>
            </View>
            <View style={{ flex: 1 }}>
