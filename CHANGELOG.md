...
- **npm packaging** — Installable via `npm install -g mcs-agent-builder` with full CLI:
  - `mcs-agent-builder start` — Launch the dashboard server
  - `mcs-agent-builder stop` — Stop a running instance
  - `mcs-agent-builder restart` — Restart the dashboard
  - `mcs-agent-builder health` — Check if running and responsive
  - `mcs-agent-builder --version` / `mcs-agent-builder --help`
  - Also accepts `--start`, `--stop`, `--restart`, `--health` flag syntax
- **`bin/cli.js`** — CLI entry point registered as `mcs-agent-builder` in npm bin
...
- **`package.json`** — Renamed from `mcs-automation` to `mcs-agent-builder`, added `bin`, `files`, `engines`, `repository`, `prepublishOnly`, `postinstall` script
- **Lockfile location** — Moved from repo root to `~/.mcs-agent-builder.lock` so it works for both local clones and global npm installs (env override via `MCS_LOCKFILE`)
